...
namespace fNIRS.Hardware.ISS
{
    ...
    //
        ...
        public uint sizoef_one_cycle_of_data;
        uint pad1;
        ...
        public ushort msg_buffer_id_number; //a number associated with this message
        ...
        //
        public fixed uint Counter[Const.max_DATAPACKET6_a2d_counters]; //Value of A2D's counters when data transfer
                                                                       //started. [counter number]
        public uint timeproc;//This is the time in microseconds between detecting
                             //when the buffer was full and posting the DataReady
                             //message (buffer processing time).

        public ulong data_block_num; //number that increases with each data blcok transferrred from the data acquistion system.
        ...
        public uint passCnt; //Number of times the timer callback function has
                             //been called

        public uint time;//Time tag, time since instance of processor was started.
                        //in us. This time is recorded just before posting the
                        //the message.
        uint pad7;

        //Information about the state of the machine.
        public ushort missed_cycles; //Number of these messages missed sinde the
                                     //current instances created, of ResetMissed called.
        public ushort unproc_msg;//Number of "_DataReady" messages posted but not
                            //responded to, when this message was posted.
                        //(Should be 0-1 unless program is too slow.)
        ushort pad8;
        ushort pad9;

        public fixed byte volt_overflow_emc[Const.max_DATAPACKET6_a2d_chns * Const.max_DATAPACKET6_ext_mux_chns];
    ...
    public unsafe struct CYCLEDATA6 
    {
	    ...
	    public fixed short rawWF[Const.max_DATAPACKET6_aqs_per_wf]; //completely raw data
        public fixed double sum_folded_rawWF[Const.max_DATAPACKET6_aqs_per_wf]; //folded raw data from

        //Aux data
        public fixed double AnalAuxADCdata[Const.max_DATAPACKET6_auxadc_chns];
        public long DigAuxADCdata;

	    //Main ADC data
        // 40 here is the size of the default 2d structure to match the target size
